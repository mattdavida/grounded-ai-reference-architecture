/*
  Azure OpenAI module — Enterprise AI Modernization Reference Architecture
  ─────────────────────────────────────────────────────────────────────────
  Provisions:
    - Azure OpenAI account (S0, the only available SKU)
    - Chat model deployment (gpt-5.4 / GlobalStandard)

  gpt-5.4 is the current production model. gpt-4o is deprecated for
  new deployments — do not revert to it.

  Capacity is in thousands of tokens per minute (TPM).
  Dev: lower capacity to control costs during POC iteration.
  Prod: higher capacity for multi-user throughput.
*/
